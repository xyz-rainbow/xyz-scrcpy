...
ensure_import_paths()
from packaging.excludes import shutil_ignore_patterns  # noqa: E402
from xyz_scrcpy import vendor_bootstrap as vb  # noqa: E402
...
def copy_project(src_root: Path, dst_root: Path) -> None:
    ...
    for lib_name in ("pkg", "lib"):
        pkg_lib = src_root / lib_name / "xyz_scrcpy"
        try:
            if pkg_lib.is_dir():
                lib_dst = dst_root / "lib" / "xyz_scrcpy"
                lib_dst.parent.mkdir(parents=True, exist_ok=True)
                shutil.copytree(pkg_lib, lib_dst)
                break
        except OSError:
            continue
    for launchers_name in ("pkg_launchers", "launchers"):
        launchers_src = src_root / launchers_name
        try:
            if launchers_src.is_dir() and any(launchers_src.iterdir()):
                shutil.copytree(launchers_src, dst_root / "launchers")
                break
        except OSError:
            continue
...
def _run_install_tui(verbose: bool) -> int:
    ...
    path = repo_root() / "bin" / "install_tui.py"
