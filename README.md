...
- Bundled upstream `scrcpy` in `vendor/` is aligned to latest stable tag `v3.3.4` and is preferred at runtime (fallback: system `scrcpy` in `PATH`). Third-party binaries in `vendor/` remain subject to their upstream licenses (see [LICENSE](LICENSE)).

### How `adb` is provided

| OS | Auto-install via this project? | What to do |
|----|-------------------------------|------------|
| **Windows** | Yes | `installer.bat` / `python -m xyz_scrcpy.setup_vendor` downloads the official scrcpy-win64 zip into `vendor/` (includes `adb.exe`). Also resolves adb from PATH, Android SDK, or `XYZ_ANDROID_PLATFORM_TOOLS`. |
| **Linux / macOS** | Yes (vendor bootstrap) | `python -m xyz_scrcpy.setup_vendor` downloads pinned platform-tools and scrcpy into `vendor/` when network is available; falls back to `apt` / `dnf` / `brew` during install. Optional: set `XYZ_ANDROID_PLATFORM_TOOLS` to a `platform-tools` directory. |

Resolution logic lives in [`lib/xyz_scrcpy/adb_resolve.py`](lib/xyz_scrcpy/adb_resolve.py). Windows diagnose: `python install_xyz.py --action diagnose`.
...
1. **Vendor download** — pinned platform-tools and screen-mirror client **v3.3.4** into `{install_dir}/vendor/` (see `vendor/NOTICE` for licenses). Multi-OS bootstrap: `python -m xyz_scrcpy.setup_vendor` (wraps [`lib/xyz_scrcpy/vendor_bootstrap.py`](lib/xyz_scrcpy/vendor_bootstrap.py)).
...
Dev clone: `./installer.sh` → **[1]** runs the same pipeline via `python -m xyz_scrcpy.setup_vendor`.
...
python -m py_compile install_xyz.py repair_xyz.py lib/xyz_scrcpy/*.py \
...
bash -n installer.sh launchers/unix/installer.sh bin/monitor.sh bin/check_and_repair.sh bin/launch_with_checks.sh scripts/validate.sh
ruff check install_xyz.py repair_xyz.py lib/xyz_scrcpy bin/ tests/ packaging/
```

One-shot local validation (same as CI):

- **Windows**: `powershell -NoProfile -File scripts\validate.ps1`
...
- `lib/xyz_scrcpy/` — installer and platform helpers (`install_xyz.py`, `adb_resolve.py`, `win_path_shim.py`, `repair_xyz.py`, `setup_vendor.py`).
- `launchers/windows/` — Windows dev menus and launcher (`installer.bat`, `xyz-scrcpy.cmd`, `start.bat`).
- `launchers/unix/` — Linux/macOS dev installer and repair shell (`installer.sh`, `repair_xyz.sh`).
- **Root entry stubs only** (everything else lives under `lib/` or `launchers/`):
  - `install_xyz.py` — Inno Setup + installer (required at repo root)
  - `repair_xyz.py` — repair workflow entry
  - `installer.bat` / `installer.sh` / `xyz-scrcpy.cmd` — one-line delegates to `launchers/`
- `scripts/clean_dev.ps1` / `scripts/clean_dev.sh` — optional dev cleanup (not bundled in the Inno EXE).
- `scripts/validate.ps1` / `scripts/validate.sh` — one-shot local CI validation.
- `scripts/enable_conhost_vt.ps1` — enables process-local VT mode for the Windows console; used by `launchers/windows/installer.bat`.
- `docs/internal/` — maintainer notes (parity matrix, implementation phases, audit reports); not shipped in releases.
