...
2. Install [uv](https://github.com/astral-sh/uv), then in the folder run `installer.bat` **or** `uv venv .venv` + `uv pip install -r .requirements.txt` + `python -m xyz_scrcpy.setup_vendor` as documented in the README.
...
5. Run the installed desktop / Start Menu launcher or `xyz-scrcpy.cmd` / `launchers/windows/start.bat` from a repo-style copy; the launcher should run pre-checks then open the menu with vendor `adb` on `PATH`.
