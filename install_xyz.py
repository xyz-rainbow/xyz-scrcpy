...
def uninstall_service(os_name: str, service_file: Path) -> None:
    if os_name == "linux":
        run_cmd(["systemctl", "--user", "disable", "--now", "scrcpy-auto.service"], check=False)
...
def open_initial_menu(os_name: str, install_dir: Path) -> None:
...
def do_uninstall(paths: dict[str, Path], os_name: str) -> None:
...
def main() -> int:
    ...
    action = args.action
    if not action:
        action = ask_choice("Choose action", ["install", "uninstall"], default="install")
    if action == "remove":
        action = "uninstall"
