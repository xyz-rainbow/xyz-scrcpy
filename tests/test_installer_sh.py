...
class InstallerShTests(unittest.TestCase):
    def test_installer_sh_passes_bash_n(self) -> None:
        ...
        for script in ("installer.sh", "launchers/unix/installer.sh", "launchers/unix/repair_xyz.sh"):
            ...
            self.assertEqual(proc.returncode, 0, msg=f"{script}: {(proc.stderr or '') + (proc.stdout or '')}")

    def test_installer_sh_content_invariants(self) -> None:
        stub = INSTALLER_SH.read_text(encoding="utf-8")
        text = UNIX_INSTALLER_SH.read_text(encoding="utf-8")
        self.assertIn("launchers/unix/installer.sh", stub)
