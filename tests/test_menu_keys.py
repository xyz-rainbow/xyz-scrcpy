...
class MenuGetKeyTests(unittest.TestCase):
    ...
    def test_get_key_delegates_escape_to_reader(self, _mock_read_esc, _os_read, mock_termios, mock_tty):
    ...
    def test_get_key_plain_char(self, _os_read, mock_termios, mock_tty):
        mock_termios.tcgetattr.return_value = MagicMock()
        with patch("sys.stdin") as mock_stdin:
            mock_stdin.fileno.return_value = 0
            result = menu.get_key()
