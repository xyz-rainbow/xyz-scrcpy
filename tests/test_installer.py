...
class InstallerTests(unittest.TestCase):
    ...
    def test_linux_launcher_includes_vendor_in_path(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Use os.sep or check for BOTH separators to be safe on cross-platform test hosts (Windows)
            self.assertTrue("/vendor" in text or "\\vendor" in text)
    ...
    def test_linux_service_unit_includes_vendor_path(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue("/vendor" in unit or "\\vendor" in unit)
